...
class BankAccount {
  ...
  constructor(client, bank, accountNumber, agencyNumber) {
    ...
    if (
      ...
      return new Error(
        `Cliente de CPF ${client.cpf} não possui conta no banco ${bank.bankName}`,
      );
    }
    this.client = client;
    this.bank = bank;
  ...
  creditAmount(amount) {
    this.#balance += amount;
    console.log(`O novo saldo da conta após o crédito é: R$ ${this.#balance}`);
  }

  debitAmount(amount) {
    this.#balance -= amount;
    console.log(`O novo saldo da conta após o débito é: R$ ${this.#balance}`);
  }
  ...
  transferTo(anotherAccount, amount) {
    ...
    if (this.bank.bankCode !== anotherAccount.bank.bankCode) {
      ...
      console.log(`Essa transferência terá uma taxa de ${this.bank.transferTax * 100}%, pois se trata de uma transferência entre bancos diferentes.`);
    }
    ...
    if (this.#balance >= amountToBeDebited) {
      ...
      console.log(`Saldo insuficiente para realizar a transferência. Seu saldo atual é de ${this.#balance}. Para realizar essa transferência você precisa ter ${amountToBeDebited} em conta.`);
    }
  }
  ...
  cashWithdrawal(amount) {
    if (amount <= this.#balance) {
      ...
      return new Error(`Saldo R$ ${this.#balance} é insuficiente para realizar a retirada de R$ ${amount}`);
    }
  }
}
