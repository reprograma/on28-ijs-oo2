...
class BankAccount {
	...
	cashWithdrawal(amount) {
		const isSuccess = this.#balance >= amount;
		const message = isSuccess
			? `A operação foi realizada com sucesso! O saldo atualizado R$ ${this.#balance -= amount}`
			: `A operação não foi realizada. Saldo insuficiente. Saldo atual da conta: R$ ${this.#balance}`;
		console.log(message);
	}
}
